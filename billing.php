...
$school_id = $_SESSION['school_id'];
    ...
    if($school['school_fee_category'] > -3 && $school['school_fee_category'] < 3) {
        ...
        ?><script type="text/javascript">
          <!--
              window.location = "billing_ind.php"
          //-->
          </script><?php }

    $stmt = $mysqli->prepare("SELECT fa_room, COUNT(fa_room) FROM facultyadvisor WHERE school_id = ? GROUP BY fa_room");
    $stmt->bind_param('d', $school_id);
    $stmt->execute();
    $fa_total;
    $stmt->bind_result($room,$count);
    while($stmt->fetch()){
        if($room == 0){
            $fa_in_single = $count;
        } else {
...
<body>
    <? getDashboardNav("billing"); ?>

    <section class="content">
        <div class="row">
            <div class="large-8 large-centered column">
                <? 
                if($school['school_num_fa'] != $fa_total){
                    ?>

                    <p> Your records indicate that the Number of faculty advisors in attendance and the number of faculty advisors whose details are in the system is not equal. You currently have <b><?echo $school['school_num_fa'];?></b>
                        faculty advisor(s) attending while there is(are) <b><?echo $fa_total;?></b> in the system. To access your bill please update your details accordingly so these numbers match. To edit Faculty Advisors in attendance, please edit
                        your delegation numbers <a href="dashboard.php">here</a>. To enter/remove faculty advisor and associated details from the system, please go <a href="delegation.php">here</a>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        echo "<tr><td>".date("m/d/Y",strtotime($school['school_registered']))."</td><td>Fee for Faculty Advisors (Single Room)</td><td>".$fa_in_single."</td><td>INR ".$global["school_fa_single_inr"]."</td><td style='text-align:right'>INR ".($fa_in_single*$global["school_fa_single_inr"])."</td></tr>";
                        echo "<tr><td>".date("m/d/Y",strtotime($school['school_registered']))."</td><td>Fee for Faculty Advisors (Double Room)</td><td>".$fa_in_double."</td><td>INR ".$global["school_fa_double_inr"]."</td><td style='text-align:right'>INR ".($fa_in_double*$global["school_fa_double_inr"])."</td></tr>";
                        $total = $delegate_fee_total + $fa_in_double*$global["school_fa_double_inr"] + $fa_in_single*$global["school_fa_single_inr"];
                        ...
                        $fa_single_total_usd = $fa_in_single * $global["school_fa_single_usd"];
                        $fa_double_total_usd = $fa_in_double * $global["school_fa_double_usd"];
                        ...
                        echo "<tr><td>".date("m/d/Y",strtotime($school['school_registered']))."</td><td>Fee for Faculty Advisors (Single Room)</td><td>".$fa_in_single."</td><td>USD ".$global["school_fa_single_usd"]."</td><td style='text-align:right'>USD ".($fa_in_single*$global["school_fa_single_usd"])."</td></tr>";
                        echo "<tr><td>".date("m/d/Y",strtotime($school['school_registered']))."</td><td>Fee for Faculty Advisors (Double Room)</td><td>".$fa_in_double."</td><td>USD ".$global["school_fa_double_usd"]."</td><td style='text-align:right'>USD ".($fa_in_double*$global["school_fa_double_usd"])."</td></tr>";
                        $total = $delegate_fee_total + $fa_in_double*$global["school_fa_double_usd"] + $fa_in_single*$global["school_fa_single_usd"];
                    ...
                    <ul class=\"accordion\" data-accordion>
                        <li class=\"accordion-navigation\">
                            ...
                            <form style = \"margin-left: 210px; margin-right: 210px; margin-top:20px;\" action=\"https://www.paypal.com/cgi-bin/webscr\" method=\"post\">
                                ...
                                <input type=\"hidden\" name=\"item_name\" value=\"ILMUNC India International School Registation Fees\">
